...
OUT_MP4 = PUBLIC_ASSETS / "bundibugyo_day25_to_65_fast_1080p.mp4"
...
def y_window(day, cases):
    ...
    pan_progress = ease_in_out((day - START_DAY) / max(latest_day - START_DAY, 1))
...
def draw_custom_axes(ax, y_min, y_max):
    ...
    ax.vlines(0, y_min, y_max, color=FG, linewidth=1.5, clip_on=True, zorder=3)
    if y_min <= 0 <= y_max:
        ax.hlines(0, 0, X_MAX, color=FG, linewidth=1.5, clip_on=True, zorder=3)
    ...
    with writer.saving(fig, str(OUT_MP4), dpi=100):
        for frame in range(frames):
            if frame < draw_frames:
                ...
                day = START_DAY + (latest_day - START_DAY) * progress
            else:
                day = latest_day
            ...
            for label, pts in comparisons.items():
                ...
                if y_min <= cy[-1] <= y_max:
                    va = "bottom" if label == "2014 West Africa" else "top"
                    offset = 18 if label == "2014 West Africa" else -16
                    ax.text(99, cy[-1] + offset, label, color=MUTED, fontsize=23, ha="right", va=va)
            ...
            fig.subplots_adjust(left=0.095, right=0.965, top=0.89, bottom=0.12)
